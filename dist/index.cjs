...
function getDataSourceInfo(dataSource) {
  return index.isFirestoreDataReference(dataSource) ||
    index.isFirestoreQuery(dataSource)
    ? ['f', dataSource.path]
    : index.isDatabaseReference(dataSource)
      ? ['r', dataSource.toString()]
      : index.isStorageReference(dataSource)
        ? ['s', dataSource.toString()]
        : []
}
...
function _useDatabaseRef(reference, localOptions = {}, isList = false) {
  ...
  if (process.env.NODE_ENV !== 'production') {
    if (
      options.target &&
      index.checkWrittenTarget(data, 'useDatabaseObject()/useDatabaseList()')
    ) {
      return data
    }
  }
...
function extractRefs(doc, oldDoc, subs, options) {
  if (!index.isPOJO(doc)) return [doc, {}]
  const dataAndRefs = [{}, {}]
  ...
  function recursiveExtract(doc2, oldDoc2, path, result) {
    ...
    for (const key in doc2) {
      ...
      if (
        ...
        // Firestore < 4.13
        ref instanceof Date ||
        ref instanceof firestore.Timestamp ||
        ref instanceof firestore.GeoPoint
      ) {
        data[key] = ref
        ...
        refs[refSubKey] = ref.converter
          ? ref
          : ref.withConverter(options.converter)
...
function subscribeToDocument(
  { ref: ref2, target, path, depth, resolve, reject, ops },
  options
) {
...
function subscribeToRefs(
  ...
  refKeys.forEach((refKey) => {
    ...
    if (sub) {
      if (sub.path !== ref2.path) sub.unsub()
      else return
    }
    subs[refKey] = {
...
function bindCollection(
  ...
  if (once) {
    ...
    stopOnSnapshot = firestore.onSnapshot(
      collection,
      onSnapshotCallback,
      reject
    )
...
function _useFirestoreRef(docOrCollectionRef, localOptions) {
  ...
  if (process.env.NODE_ENV !== 'production') {
    if (
      options.target &&
      index.checkWrittenTarget(data, 'useDocument()/useCollection()')
    ) {
      return data
    }
  }
  ...
  const initialValue = getInitialValue(
    ...
    NO_INITIAL_VALUE,
    /* CUSTOMIZATION START */
    docOrCollectionRef?.firestore?.app || index.useFirebaseApp()
    /* CUSTOMIZATION END */
  )
  ...
  function bindFirestoreRef() {
    ...
    const newPromise = new Promise((resolve, reject) => {
      ...
      unbind = (
        index.isDocumentRef(docRefValue) ? bindDocument : bindCollection
      )(
  ...
  let stopWatcher = index.noop
  if (
    vueDemi.isRef(docOrCollectionRef) ||
    typeof docOrCollectionRef === 'function'
  ) {
...
function databasePlugin(app, pluginOptions, firebaseApp) {
  ...
  GlobalTarget[bindName] = function databaseBind(key, source, userOptions) {
    ...
    const scope = index
      .getGlobalScope(firebaseApp || index.useFirebaseApp(), app)
      .run(() => vueDemi.effectScope())
...
const firestorePlugin = function firestorePlugin2(
  app,
  pluginOptions,
  firebaseApp
) {
...
function _VueFireAuthInit(
  firebaseApp,
  app,
  initialUser,
  dependencies,
  auth$1 = auth.initializeAuth(firebaseApp, dependencies)
) {
  const user = index
    .getGlobalScope(firebaseApp, app)
    .run(() => vueDemi.ref(initialUser))
